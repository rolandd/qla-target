...
void generic_target_put_session(struct se_session *se_sess)
{
	if (se_sess->se_tpg->se_tpg_tfo->put_session)
		se_sess->se_tpg->se_tpg_tfo->put_session(se_sess);
	else
		...
		u32 data_length, int task_attr, int data_dir, int flags)
{
	 ...
	 */
	if (transport_lookup_cmd_lun_cdb(se_cmd, unpacked_lun, cdb) < 0) {
...
void target_put_sess_cmd(struct se_session *se_sess, struct se_cmd *se_cmd)
{
	...
	kref_put_and_lock(&se_cmd->cmd_kref, target_release_cmd_kref,
...
void target_sess_cmd_list_set_waiting(struct se_session *se_sess)
{
	...
	WARN(se_sess->sess_tearing_down, "se_sess %p [initiator %s]: sess_tearing_down already set\n",
	     se_sess, se_sess->se_node_acl ? se_sess->se_node_acl->initiatorname : "??");
