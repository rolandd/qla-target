...
static int e1000_request_msix(struct e1000_adapter *adapter)
{
	...
	err = request_irq(adapter->msix_entries[vector].vector,
			  e1000_intr_msix_rx, IRQF_SAMPLE_RANDOM, adapter->rx_ring->name,
			  netdev);
	if (err)
		goto out;
	...
	err = request_irq(adapter->msix_entries[vector].vector,
			  e1000_intr_msix_tx, IRQF_SAMPLE_RANDOM, adapter->tx_ring->name,
			  netdev);
	if (err)
		goto out;
	...
	err = request_irq(adapter->msix_entries[vector].vector,
			  e1000_msix_other, IRQF_SAMPLE_RANDOM, netdev->name, netdev);
	if (err)
		goto out;
...
static int e1000_request_irq(struct e1000_adapter *adapter)
{
	...
	if (adapter->flags & FLAG_MSI_ENABLED) {
		err = request_irq(adapter->pdev->irq, e1000_intr_msi, IRQF_SAMPLE_RANDOM,
				  netdev->name, netdev);
		if (!err)
			return err;
	...
	err = request_irq(adapter->pdev->irq, e1000_intr, IRQF_SHARED | IRQF_SAMPLE_RANDOM,
			  netdev->name, netdev);
	if (err)
...
static int e1000_test_msi_interrupt(struct e1000_adapter *adapter)
{
	...
	err = request_irq(adapter->pdev->irq, e1000_intr_msi_test, IRQF_SAMPLE_RANDOM,
			  netdev->name, netdev);
	if (err) {
		pci_disable_msi(adapter->pdev);
		goto msi_test_failed;
